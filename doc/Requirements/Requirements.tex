...
Being that YardSale is an open source projectv the initial
...
\section{YardSale Database Design}
	...
	\subsection{On-screen Keyboard}
    \includegraphics{ys_keybrd_screener.png}\\
	
	The On-Screen Keyboard is a way for the user to enter standard
	text into the system when necessary. Like the on-screen
	calculator it can be called from anywhere within the program via
	the toolbar to enter text into a field. It provides the user with
	a familiar keyboard design with letter, number, shift, delete and space buttons.
	As well as pressing the buttons on the touch-screen corresponding 
	to the keys, a user can type directly onto an attached keyboard and the
	text will appear in the on-screen keyboard's display. When the user
	is finished typing they press the 'done' button to close the keyboard 
	and the value is entered into the field they were editing.
