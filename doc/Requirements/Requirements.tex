...
\chapter{Requirements Definition}
    ...
    \section{Introduction}
        ...
        way in which it is designed.  A user of the YardSale system
        ...
        with us to improve the functionality of their existing POS
    ...
    \section{Functional Requirements}
        ...
        \subsection{Point of Sale Transactions}
            ...
            \begin{itemize}
                ...
                \begin{itemize}
                    ...
                    in the system.
                    \item {\bf Inventory Information:} Includes all information
        ...
        \subsection{Reporting}
            ...
            \begin{itemize}
                ...
                each employee logged to the system.
    ...
    \section{Preliminary Design}
        ...
        \subsection{System Architecture}
            ...
            \begin{center}
                ...
                \figurename{ 1.1: System Architecture Flow Chart}
            \end{center}
...
\newpage
        ...
        \subsection{Team Member Roles and Responsibilities}
            ...
            \begin{center}
                ...
                \figurename{ 1.2: Hierarchy of Team Roles and
                Responsibilites}
            \end{center}

            \noindent The team roles are defined as follows:
...
\chapter{Design}
    ...
    The following sections describe the database level design and the client
    application level design of YardSale. A database was chosen to store the
    ...
    MySQL 4.  Since all of our queries are based on the
    SQL standard, however, it should be easy to allow the use of any
...
\section{YardSale Database Design}
    ...
    the following diagram has been reduced in size to the point of
    making it impossible to read, the table relations can be seen,
    as well as table specifics like field values, indexes, and foreign and primary keys.
...
%        \includegraphics{Database_Layout_REDUCED.png}\\
        \figurename{ 2.1: Global Database Structure}
    \end{center}
    ...
    to a numbered table in the diagram. The keys associated with each table are listed as well.\\
    ...
    \newpage
        ...
        Since customer management is a simple task it is maintained in only
        one table. There is no real need to have this data stored across many
...
%            \includegraphics{Tables/CustomerTable.png}\\
            \figurename{ 2.2: Database Customer Table (UML)}
        \end{center}
        ...
        transactions. By allowing this association YardSale will later be able to
        formulate reports on how much each of its customers spend, for example. It also
        provides an in-depth directory listing of all of a business's clients, for use
        in any form the company may see fit.
        ...
        First, Middle, and Last names are all stored in separate fields of their own,
        as is the Address information. Credit information can also be stored about
        users,
        but is optional; it is also stored in the field as an encrypted
        string of data so that underprivileged users cannot access it. Two interesting
        features about this table is the ability to store a link to a photograph and
        signature for each customer. This will aid in positively identifying a
        customer when he or she is buying items with a check or credit card.
...
%            \includegraphics{Tables/InventoryTable.png}\\
            \figurename{ 2.3: Database Inventory Table (UML)}
        \end{center}

        This table is the main Inventory storage that structure contains all information needed
        ...
        with each item they produce, so their is a field available for that as well.
        ...
        item is received it updates the field corresponding to the same item's last shipment. Some items are
        oddly shaped or are excessively heavy; both of these two options could cause the oversized
...
%        \includegraphics{Tables/TaxTable.png}\\
        \figurename{ 2.4: Database Tax Table (UML)}
        \\
        \\
...
%        \includegraphics{Tables/VendorTable.png}\\
        \figurename{ 2.5: Database Vendor Table (UML)}
        \\
        \\
    ...
    \newpage
        ...
        tables. Transaction handling is split into two sections. The first section is the actual
        day to day transaction, and the second is the added functionality of packaging the items sold during
        the transaction.\\
        \\
        \\
...
%        \includegraphics{Tables/TransactionLogTable.png}\\
        \figurename{ 2.6: Database Transaction Log Table (UML)}
        \\
        \\
...
%        \includegraphics{Tables/PackageTable.png}\\
        \figurename{ 2.7: Database Package Table (UML)}
        \\
        \\
...
%        \includegraphics{Tables/CarrierTable.png}\\
        \figurename{ 2.8: Database Carrier Table (UML)}
        \\
        \\
...
%        \includegraphics{Tables/ShippingTable.png}\\
        \figurename{ 2.9: Database Shipping Table (UML)}
        \\
...
%        \includegraphics{Tables/EmployeeTable.png}
        \figurename{ 2.10: Database Employee Table (UML)}
        \\
        \\
        ...
        field in this table is the password field which is used to store a user's password to allow
 ...
 %       \includegraphics{Tables/LoginTable.png}\\
        \figurename{ 2.11: Database Login Table (UML)}
        \\
        \\
...
%        \includegraphics{Tables/ACL.png}\\
        \figurename{ 2.12: Database Access Control Table (UML)}
        \\
        \\
        The ACL table which, is short for Access Control List Table, manages which user types have access
        to different levels of program use. This table contains the Name of a user type and a description
        of its functionality. Currently we foresee only four user types and will likely not have an
...
%        \includegraphics{Tables/KeyTable.png}\\
        \figurename{ 2.13: Database Key Table (UML)}
        \\
        \\
    ...
    \figurename{ 2.14: YardSale Main Menu Screen}\\
    ...
    \figurename{ 2.15: YardSale Sale Transaction Screen}\\

    The Sale Transaction Screen is designed to provide much information and functionality, in
    ...
    \figurename{ 2.16: YardSale Inventory Management Screen}\\

    The Inventory Management Screen can display information pertaining to all
    ...
    screen is only available to manager-access-level users
    and higher.\\

    \subsection{Employee Management Screen}
    \includegraphics{ys_employee_screener.png}\\
    \figurename{ 2.17: YardSale Employee Management Screen}\\
    ...
    information is displayed.  It is only available to
    ...
    \figurename{ 2.18: YardSale On-Screen Calculator}\\
...
testing.
    ...
    The passive testing of YardSale was accomplished by our auto build system.  The auto build system pulled the latest source
    code from the repository and compiled it with the most strict settings.  If the code failed the system reported the error
    on our website.  The auto build system also compiled the code on six other architechures including Amd64, Sparc, Mac OSX,
    Linux, and Microsoft Windows.  The extensive compilation of the source code ensured that no platform dependant source code
    violated the portability of the project.
    ...
    Each component that utilizes the database is tested by creating ``dummy" data which is loaded directly into the database backend.  Then each database interface screen is tested by logging into YardSale client and assuring that each dataset is loaded correctly.  Inserts into the database are verified through a third-party database interface tool.
    ...
    \begin{itemize}
        ...
        fail, the build system will report an error.
            ...
            The YardDatabase class contains a testing main which connects to a ``dummy" testing database.
            If the connection succeeds, the method to tries modify an inventory item that has been successfully
            ...
            The YardDatabase class contains a testing main which connects to a ``dummy" testing database.
            Once successfully connected the method, it will search for pre-loaded items by SKU number and description.
            ...
            After the ``delete" command is sent, the method will try to retrieve that deleted item. If this retrieve works
        ...
        The YardDatabase class contains a testing main which connects to a ``dummy" testing database.
         Once successfully connected, the method will search for pre-loaded items by name and date.
            ...
            The YardDatabase class contains a testing main which connects to a ``dummy" testing database.
            If the connection succeeds, the method tries to modify a customer that has been successfully
    ...
    \section {Results}
    \subsection{ Passive Testing }
    The end result of our passive testing system was to mold our code into an optimal
    subset of ANSI C++.  We focused on two main compilers for our testing: GCC 3.x
    (which is the GNU Compiler Collection which we used to compile on Linux, FreeBSD,
    and Solaris) and Visual C++ .NET 2003 (on Microsoft Windows platforms).

    The main problems we encountered with passive testing where that different compilers
    gave different errors for the same code.  For instance, GCC 3.x will allow a
    function that returns a value to not explicitly say 'return variable' in the code
    if the correct variable type is defined in the top of the function (GCC assumes this
    variable will be returned); on the other hand, VC++ .NET gives an explicit error here.
    In all honesty, the VC++ behaviour was more helpful in our testing.

    Another instance of the compilers disagreeing is in VC++ .NET's template compliance.
    In VC++ .NET, the entire function body of a member template function must be defined
    in the class header.  This behavior is sometimes counter-productive and makes
    headers larger then they should be in some cases.

    \subsection { Active Testing }
    Our active testing methods were essencial to our project's success.
    The testing mains of each of our classes proved to be especially
    useful when we transfered our system over to XMLNode-base trees.
    The disadvantage of using XML is that all nodes of a tree are
    referenced by a string name.  Because of this, the compiler cannot
    catch errors in the spelling of a name.  Thankfully, the testing
    mains provide that much-needed error checking.
