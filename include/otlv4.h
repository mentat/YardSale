// ==============================================================
// Oracle, ODBC and DB2/CLI Template Library, Version 4.0.76,
...
#define OTL_VERSION_NUMBER (0x04004CL)
...
#define OTL_TRACE_NEXT_FETCH                                                    \
   if(OTL_TRACE_LEVEL & 0x4 && cur_row==0){                                     \
     OTL_TRACE_STREAM<<OTL_TRACE_LINE_PREFIX                                    \
...
#define OTL_TRACE_STREAM_EXECUTION                              \
   if(OTL_TRACE_LEVEL & 0x4){                                   \
     OTL_TRACE_STREAM<<OTL_TRACE_LINE_PREFIX                    \
                     ...
                     <<stm_text<<", buffer size="<<array_size   \
                     <<"\n";                                    \
   }
#endif
...
#define OTL_TRACE_STREAM_EXECUTION2                                     \
    if(OTL_TRACE_LEVEL & 0x4){                                          \
      OTL_TRACE_STREAM<<OTL_TRACE_LINE_PREFIX                           \
                      <<"otl_stream, executing SQL Stm="<<stm_text      \
...
public:
 ...
 void rlogon(const char* connect_str,const int auto_commit=0)
 {
   OTL_TRACE_FUNC(0x1,
                  "otl_connect",
                  "rlogon",
                  "connect_str=\""<<connect_str<<"\", auto_commit="<<auto_commit)
  throw_count=0;
...
public:
 ...
 {int j;
  ...
  for(i=1;
      this->describe_column(sl_desc_tmp[i-1],i);
      ++i)
    ++sld_tmp_len;
  sl_len=sld_tmp_len;
  if(sl){
...
#if defined(USER_DEFINED_STRING_CLASS) || defined(OTL_STL)
      s.assign((char*)c,len);
#elif defined(OTL_ACE)
      s.set((char*)c,len,1);
#endif
...
public:
   ...
   OTL_THROWS_OTL_EXCEPTION
 {
  ...
  s.fraction=otl_from_fraction(tmp.fraction,s.frac_precision);
#endif
  OTL_TRACE_WRITE(s.month<<"/"<<s.day<<"/"<<s.year<<" "<<s.hour<<":"<<s.minute<<":"<<s.second<<"."<<s.fraction,
                  "operator >> ",
                  "otl_datetime&");
  inc_next_ov();
  return *this;
 ...
 last_oper_was_read_op=false;
  ...
  (*this)<<tmp;  
    OTL_TRACE_READ(s.month<<"/"<<s.day<<"/"<<s.year<<" "<<s.hour<<":"<<s.minute<<":"<<s.second<<"."<<s.fraction,
                  "operator >> ",
                  "otl_datetime&");
...
#endif
   OTL_TRACE_WRITE(n,"opeartor >> ","int&")
   inc_next_ov();
   return *this;
 }
...
#endif
   OTL_TRACE_WRITE(u,"opeartor >> ","unsigned&")
   inc_next_ov();
   return *this;
 }
...
#endif
   OTL_TRACE_WRITE(sh,"opeartor >> ","short int&")
   inc_next_ov();
   return *this;
 }
...
#endif
   OTL_TRACE_WRITE(l,"opeartor >> ","long int&")
   inc_next_ov();
   return *this;
 }
...
public:
   ...
   OTL_THROWS_OTL_EXCEPTION
 {
  last_oper_was_read_op=false;
   reset_end_marker();
   OTL_TRACE_READ(sh,"operator <<","shotr int")
  if((*io))
   (*io)->operator<<(sh);
  else if((*ss))
   (*ss)->operator<<(sh);
...
otl_stream& operator>>(otl_stream& s, OTL_BIGINT& n)
  OTL_THROWS_OTL_EXCEPTION
{
...
otl_stream& operator<<(otl_stream& s, const OTL_BIGINT n)
  OTL_THROWS_OTL_EXCEPTION
{
  char temp_val[otl_bigint_str_size];
